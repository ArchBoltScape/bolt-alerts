<script lang="ts">
  ...
  let alertList: Writable<{[id: string]: Ruleset}> = ((q) => {
    ...
    return writable(alertConfigList.map((x) => {return {
      id: x.id,
      name: x.name,
      rules: {},
      expanded: false,
...
<main>
  ...
  <div class="m-[7px] w-auto h-auto">
    ...
    <!-- svelte-ignore a11y-no-static-element-interactions -->
    <div class="m-0 p-0 b-0 absolute left-[5px] top-[5px] right-[23px] h-[18px] bg-[grey] text-center text-[10pt]" onmousedown={(e) => {if (e.button === 0) startDrag('0', '0')}}>
