...
local convert = function (s)
  return string.gsub(s, ".", function (c) return string.format("\\x%02x", string.byte(c)) end)
end

local chatpixels = require("chatpixels")
local heightmap = {}
for key, val in pairs(chatpixels) do
  local height = 0
  for _ in pairs(val) do height = height + 1 end
  local h = heightmap[height]
  if h then
    h[key] = val
  else
    heightmap[height] = {[key] = val}
  end
end

local heightmapfrequencies = {}
for height, v in pairs(heightmap) do
  local t = {}
  for i = 0, (height - 1) do
    local table = {}
    for key, val in pairs(v) do
      local char = string.sub(key, 1, 1)
      local pt = string.sub(key, 3, #key)
    
      local pixels = val[i]
      local e = table[pixels]
      if e then
        local ec = e[char]
        if ec then
          ec[pt] = true
        ...
        table[pixels] = {[char] = {[pt] = true}}
      end
    end
    t[i] = table
  end
  heightmapfrequencies[height] = t
end

-- heightmapfrequencies is: {[height] = {[row] = {[pixeldata] = {['a'] = {['12'] = true}, ['b'] = {['10'] = true, ['14'] = true}, ...}, ...}, ...}, ...}
local str = ""
for height, val in pairs(heightmapfrequencies) do
  local lowesthigh = nil
  local lowestrow
  for row, v in pairs(val) do
    local highestcount = 0
    for _, t in pairs(v) do
      local count = 0
      for c, _ in pairs(t) do count = count + 1 end
      if count > highestcount then highestcount = count end
    end
    if not lowesthigh or highestcount < lowesthigh then
      lowesthigh = highestcount
      lowestrow = row
    end
  end

  str = str .. string.format("[%s] = {\n  row = %s,\n  data = {\n", height, lowestrow)
  for pixeldata, t in pairs(val[lowestrow]) do
    -- t: {['a'] = {['12'] = true}, ['b'] = {['10'] = true, ['14'] = true}, ...}
    local count = 0
    local first = nil
    local last = nil
    for k in pairs(t) do
    ...
    if count == 1 then
      local needescape = first == '\'' or first == '\\'
      str = str .. string.format("[\"%s\"] = '%s',\n", convert(pixeldata), needescape and ('\\' .. first) or first)
    elseif count == 2 then
      local allcharnames = {}
      for key in pairs(t[first]) do
        allcharnames[string.format("%s-%s", first, key)] = true
      end
      for key in pairs(t[last]) do
        allcharnames[string.format("%s-%s", last, key)] = true
      end

      local s = ""
      for k in pairs(allcharnames) do s = s .. k .. ", " end
      print(string.format("trying to distinguish: %s", s))

      local distinctrow = nil
      for i = 0, (height - 1) do
        print(string.format("distinguishing on row %s", i))
        local t = {}
        local isdistinct = true
        for charname in pairs(allcharnames) do
          local p = heightmap[height][charname][i]
          if t[p] then
            print(string.format("row %s isn't distinct because we've already seen the data for '%s'", i, charname))
            isdistinct = false
            break
          else
            print(string.format("adding data for '%s'", charname))
            t[p] = true
          end
        end
        if isdistinct then
      ...
      if distinctrow == nil then
        error(string.format("no distinct row: %s", s))
      end

      str = str .. string.format("    [\"%s\"] = {\n      row = %s,\n      data = {\n", convert(pixeldata), distinctrow)
      for charname in pairs(allcharnames) do
        str = str .. string.format("        [\"%s\"] = '%s',\n", convert(heightmap[height][charname][distinctrow]), string.sub(charname, 1, 1))
      end
      str = str .. "      },\n    },\n"
    else
      error("too many possible letters")
    end
  end
  str = str .. "  },\n},\n"
end

bolt.saveconfig("stuff.lua", str)
